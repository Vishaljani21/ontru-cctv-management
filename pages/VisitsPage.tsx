        ...
        <div className="lg:col-span-1">
          <div className="p-6 bg-white border border-slate-200 rounded-lg shadow-sm">
            ...
            <form onSubmit={handleCreateVisit} className="space-y-4">
              ...
              <div>
                <label htmlFor="technician" className="block text-sm font-medium text-slate-700">Assign Technician</label>
                <select id="technician" value={technician} onChange={e => setTechnician(e.target.value)} required className={`mt-1 block w-full px-3 py-2 border border-slate-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 ${!technician ? 'text-slate-500' : 'text-slate-900'}`}>
                  <option value="" disabled>Select a technician</option>
                  {technicians.map(tech => (
                    <option key={tech.id} value={tech.id} className="text-slate-900">{tech.name}</option>
                  ))}
                </select>
              </div>
