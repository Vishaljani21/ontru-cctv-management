    ...
    return (
        <div className="space-y-8 animate-fade-in-up">
            ...
            <div className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-4">
                <div>
                    <h2 className="text-3xl font-bold text-slate-800 dark:text-white tracking-tight">Suppliers</h2>
                    <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Manage sales persons, distributors, and service center contacts.</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredSuppliers.length > 0 ? filteredSuppliers.map((supplier) => (
                    <div key={supplier.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden flex flex-col group p-5">
                        <div className="flex justify-between items-start mb-4">
                            ...
                            <div className="h-10 w-10 flex-shrink-0 rounded-full bg-primary-50 dark:bg-primary-900/20 flex items-center justify-center text-primary-600 dark:text-primary-400 font-bold text-lg">
                        ...
                        <div className="space-y-3 flex-grow">
                            {supplier.sales_person_name && (
                                <div className="flex items-start gap-3">
                                    <div className="mt-1"><svg className="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg></div>
                                    <div>
                                        <p className="text-sm font-medium text-slate-700 dark:text-slate-200">{supplier.sales_person_name} <span className="text-xs text-slate-400">(Sales)</span></p>
                                        {supplier.sales_person_mobile && <a href={`tel:${supplier.sales_person_mobile}`} className="text-xs text-primary-600 dark:text-primary-400 hover:underline block">{supplier.sales_person_mobile}</a>}
                                    </div>
                                </div>
                            )}
                            ...
                            {supplier.manager_name && (
                                <div className="flex items-start gap-3">
                                    <div className="mt-1"><svg className="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg></div>
                                    <div>
                                        <p className="text-sm font-medium text-slate-700 dark:text-slate-200">{supplier.manager_name} <span className="text-xs text-slate-400">(Manager)</span></p>
                                        {supplier.manager_mobile && <a href={`tel:${supplier.manager_mobile}`} className="text-xs text-primary-600 dark:text-primary-400 hover:underline block">{supplier.manager_mobile}</a>}
                                    </div>
                                </div>
                            )}
                            ...
                            {(supplier.service_center_number || supplier.service_center_details) && (
                                <div className="flex items-start gap-3 pt-2 border-t border-slate-100 dark:border-slate-800">
                                    ...
                                    <div>
                                        <div className="text-xs font-bold text-slate-500 uppercase">Service Center</div>
                                        {supplier.service_center_number && <a href={`tel:${supplier.service_center_number}`} className="text-sm font-medium text-slate-700 dark:text-slate-200 block">{supplier.service_center_number}</a>}
                                        {supplier.service_center_details && <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 line-clamp-2">{supplier.service_center_details}</p>}
                                    </div>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="flex justify-end gap-2 pt-4 border-t border-slate-100 dark:border-slate-800 mt-4 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => handleDelete(supplier.id)} className="p-2 text-red-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors" title="Delete">
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                            </button>
                            <button onClick={() => handleOpenModal(supplier)} className="p-2 text-slate-400 hover:text-primary-600 dark:hover:text-primary-400 hover:bg-primary-50 dark:hover:bg-primary-900/20 rounded-lg transition-colors" title="Edit">
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                            </button>
                        </div>
                    </div>
                )) : (
                    <div className="col-span-full py-12 text-center text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-900 rounded-2xl border border-dashed border-slate-200 dark:border-slate-700">
                        <p className="text-lg font-medium">No suppliers found</p>
                        <p className="text-sm mt-1">Add your supplier contacts to manage them here.</p>
                    </div>
                )}
            </div>
        </div>
