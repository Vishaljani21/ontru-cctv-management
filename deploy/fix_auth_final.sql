...
-- MANUAL AUTH SCHEMA REPAIR v5 (The "Sync" Fix)
-- 1. Sets Search path for 'postgres' so it sees the tables.
-- 2. Backfills ALL migration history so GoTrue knows they are already created.

-- A. Fix Search Path for 'postgres' 
...
-- B. Backfill Migration History (CRITICAL)
-- These match the schema we manually created. GoTrue must skip them.
...
DO $$
BEGIN
    RAISE NOTICE 'FIX APPLIED: Search Path set + Migrations Synced.';
END $$;
